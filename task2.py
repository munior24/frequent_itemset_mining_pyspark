...
if __name__ == '__main__':
    ...
    interm_file = './interm.csv'
    sc = SparkContext.getOrCreate()
    text_rdd = sc.textFile(input_csv_path)
    ...
    with open(interm_file, 'w') as output:
    ...
    raw_rdd = sc.textFile(interm_file, partitions)
    header = raw_rdd.first()
    data = raw_rdd.filter(lambda line: line != header)
    basket_rdd = data.map(lambda x: x.split(",")).map(lambda x: (x[0], x[1])).groupByKey().mapValues(set).filter(lambda x: len(x[1]) > int(filter_threshold)).map(lambda x: x[1]).persist()
    # basket_rdd = get_basket_rdd(data, 1)
    size = basket_rdd.count()
